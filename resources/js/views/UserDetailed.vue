<template>
    ...
    <v-card class="mt-4 pa-2">
        <v-card v-if="Manager" class="my-4">
            <v-card-title>
                <b>{{ $t("Chairman") }}:</b>{{ Manager.name }}
            </v-card-title>
            <v-card-text>
                <div></div>
                <div>
                    <b>{{ $t("Phone") }}:</b> {{ Manager.phone }}
                </div>
                <div>
                    <b>{{ $t("Email") }}:</b> {{ Manager.email }}
                </div>
            </v-card-text>
        </v-card>
        <v-card class="mt-4">
            ...
            <v-card-text>
                <v-form ref="form" v-model="valid">
                    ...
                    <v-alert v-if="amount" color="warning">
                        ...
                        გადასახდელი იქნება {{ realAmount }}ლარი
                    </v-alert>
                </v-form>
            </v-card-text>
...
export default {
    ...
    computed: {
        realAmount() {
            ...
            //  თუ amount არის ფლოატი ვიღებთ პირველად დაფიქსულ ვალიუს, ვამრგვალებთ მაღლა და ისევ ვფქისავთ
            if (!Number.isInteger(this.amount)) {
                //  ზემოთ მრგვალდება იმ შემთხვევაში თუ წერტილის შემდეგ ფლოატი 0 ზე მეტია

                result = Math.ceil(result * 100).toFixed(2);
            }
            return result;
        },
        chartOptions() {
    ...
    methods: {
        ...
        getCards() {
            axios
                .get("/api/user/cards/" + this.$route.params.id)
                .then(({ data }) => {
                    this.desserts = data;
                });
        },
        ...
        updateCard() {
            axios
                .put("/api/cards/" + this.cardEdit.id, this.cardEdit)
                .then(() => {
                    this.getCards();
        ...
        updatePhone() {
            axios
                .put(`/api/users/${this.user.id}`, this.userUpdate)
                .then(() => {
                    this.signIn();
