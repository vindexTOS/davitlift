            ...
            <v-card-title class="d-flex align-center justify-space-between">
                ...
                <v-menu>
                    ...
                    <v-card width="250" class="pa-0 ma-0">
                        <v-btn style="width: 100%" @click="dialog = true">{{$t('Add lift')}}</v-btn>
                    </v-card>
                </v-menu>
            </v-card-title>
            <v-card-text>
    ...
    computed: {
        ...
        serverItemsFilter() {
            ...
            return this.serverItems?.filter(x => {
                if(x.name.includes(this.search) || x.user.email.includes(this.search) || x.dev_id.includes(this.search)) {
                    if(!this.isActive && !this.notActive && !this.hasError && !this.deleted) return x
