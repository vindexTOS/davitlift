<template>
    <div v-if="data.companies">
        <v-card class="pa-2 mb-2 ">
            <v-card-title><h3>{{ $t('Companies') }}</h3>
                <h5>
            ...
            <v-row class="justify-space-between">
                <v-col v-if="data['totalEarning']" style="min-height: 100%" cols="12" md="6">
                ...
                <v-col cols="12" md="6">
                    <v-card style="height: 100%" class="overflow-auto pa-2">
                        <span>{{ $t('Condition of elevators') }} : {{ seriesC[0] + seriesC[1] }}</span>
                        <apexchart width="400" height="350" type="donut" :options="chartOptionsC"
                                   :series="seriesC"></apexchart>
                    </v-card>
            ...
            <v-row class="justify-space-between">
                ...
                <v-col v-if="data['totalServiceFee'] > 1" cols="12" md="6">
                    <v-card style="height: 100%" class="overflow-auto pa-2">
                        <span>{{ $t('ჯამური სერვისის გადასახადი') }} : {{
                                (data['totalServiceFee']).toFixed(2)
                            }}{{ $t('Lari') }}</span>
                        <apexchart width="400" height="350" type="donut" :options="chartOptionsB"
        ...
        >
            ...
            <template v-slot:item.company_name="{item}">
                <RouterLink :to="`/company/${item.raw.id}`">{{ item.raw.company_name }}</RouterLink>
            </template>
        ...
        <v-card>
            ...
            <v-card-text>
                ...
                <v-col
                    cols="12"
                >
                    <p>{{ $t('The maximum amount to be enrolled') }}:
                        {{ (serviceFee.max - serviceFee.company.withdrewedTotalFee).toFixed(2) }} {{ $t('Lari') }}</p>
    ...
    methods: {
        ...
        loadItems() {
            axios.get(`/api/companies`).then(({data}) => {
                ...
                this.seriesC = [this.data.deviceStats.inactive, this.data.deviceStats.active,]
                this.seriesB = [Number(((data['totalServiceFee']).toFixed(2) - data['payedServiceFee']).toFixed(2)), Number(data['payedServiceFee'])]
        ...
        editItem(item) {
            ...
            this.editedItem = Object.assign({}, {admin_email: item.admin.email, ...item})
            this.dialog = true
        },
        ...
        async save() {
            ...
            if (this.editedItem.id) {
                await axios.put('/api/companies/' + this.editedItem.id, this.editedItem)
            } else {
                await axios.post('/api/companies', this.editedItem)
            }
            this.$swal.fire({
                icon: 'success',
                position: 'center',
