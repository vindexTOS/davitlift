<template>
    <v-card v-if="data.company" class="mb-3">
        ...
        <v-card-text>
            <v-card>
                ...
                <v-card-text>
                    <div>
                        <b>{{$t('Name')}}:</b> {{ data.company.admin.name }}
                    </div>
                    <div>
                        <b>{{$t('Phone')}}:</b> {{ data.company.admin.phone }}
                    </div>
                    <div>
                        <b>{{$t('Email')}}:</b> {{ data.company.admin.email }}
                    </div>
                </v-card-text>
            </v-card>
            <h4>
        ...
        <v-card class="pa-2">
            ...
            <v-row class="justify-space-between">
                <v-col v-if="seriesD[0] + seriesD[1]" style="min-height: 100%" cols="12" md="6">
                    <v-card style="height: 100%" class="overflow-auto pa-2">
                        <h3>{{$t('Cashback')}} </h3>
    ...
    <div class="mt-3">
        <CashbackTable :companyId="$route.params.id" @getCashback="loadItems" :max-cashback="((data['companyFee']/100).toFixed(2) - data['payedCompanyFee']).toFixed(2)" :isCompanyPage="true" :server-items="data['companyTransaction']"></CashbackTable>
    </div>
</template>
<script>
...
export default {
    components: {
        CashbackTable,
        apexchart: VueApexCharts, ManagersTable
    },
    data: function () {
        return {
            data: {},
    ...
    methods: {
        loadItems() {
            axios.get('/api/companies/' + this.$route.params.id).then(({data}) => {
                ...
                this.seriesC = [Number((this.data.companyFee - this.data.payedCompanyFee).toFixed(2)),Number(this.data.payedCompanyFee)];
                this.seriesD = [Number((Number(this.data['needToPay'])- Number(this.data['payedCashback'])).toFixed(2)),Number(this.data['payedCashback'].toFixed(2))];
