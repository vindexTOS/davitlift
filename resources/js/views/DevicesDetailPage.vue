      ...
      'pa-0' : $vuetify.display.xs
        ...
        <v-card class="pa-sm-2">
            <template v-slot:title>
                <div v-if="$vuetify.display.mdAndUp" class="d-flex justify-space-between">
                    <div class="d-flex">
                        ...
                        <v-chip
                            v-if="new Date(data.lastBeat).getTime() > new Date().getTime()"
                            class="ma-2"
                            color="green"
                            text-color="white"
                        >
            ...
            <v-card-text>
                <div class="d-sm-flex justify-space-between">
                    <v-col class="px-0" cols="12" sm="6">
                        <v-card>
                            <v-card-text>
                                ...
                                <h4>{{ $t('Phone') }}: {{ data.user.phone }}</h4>
                                <h4>{{ $t('Email') }}: {{ data.user.email }}</h4>
                            </v-card-text>
                        </v-card>
                    </v-col>
                    <v-col class="px-0" style="min-height: 100%;" cols="12" sm="6">
                        <v-card style="min-height: 100%;">
                            <v-card-text>
                                <h3>{{ $t('Statistics') }} </h3>
                                <h4>{{ $t('Total amount earned') }}: {{ earnings / 100 }} {{ $t('Lari') }}</h4>
                ...
                <v-expansion-panels class="mb-3">
                    ...
                    >
                        ...
                        <template v-slot:text>
                            <h4>{{ $t('Mode of payment') }}: {{
                                    data.op_mode == 1 ? $t('Tariff') + data.tariff_amount + $t('Tetri') : this.$t('Fixed') + data.tariff_amount / 100 + this.$t('Lari')
                                }} </h4>
                            ...
                            <h4
                                v-if="$store.state.auth.user.lvl >= 4"
                            >{{ $t('LCD brightness') }}: {{ data.lcd_brightness }}%</h4>
                            <h4
                                v-if="$store.state.auth.user.lvl >= 4"
                            >{{ $t('LED brightness') }}: {{ data.led_brightness }}%</h4>
                            <h4
                                v-if="$store.state.auth.user.lvl >= 4"
                            >{{ $t('Card reading time') }}: {{ data.card_read_delay }}{{ $t('Second') }}</h4>
                            <h4
                                v-if="$store.state.auth.user.lvl >= 4"
                            >{{ $t('Message display time') }}: {{ data.msg_appear_time }}{{ $t('Second') }}</h4>
                            <h4
                                v-if="$store.state.auth.user.lvl >= 4"
                            >{{$t('Relay pulse time')}}: {{data.relay_pulse_time}}{{ $t('Second') }}</h4>
                            <h4
                                v-if="$store.state.auth.user.lvl >= 4"
                            ...
                            >{{ $t('The second line of the guest message') }}:
                                {{ englishToGeorgian(data.guest_msg_L2) }}</h4>
                            <h4
                                v-if="$store.state.auth.user.lvl >= 4"
                            ...
                            <h4
                                v-if="$store.state.auth.user.lvl >= 4"
                            >{{ $t('The first line of the validation message') }}:
                                {{ englishToGeorgian(data.validity_msg_L1) }}</h4>
                            <h4
                                v-if="$store.state.auth.user.lvl >= 4"
                            >{{ $t('The second line of the validation message') }}:
                                {{ englishToGeorgian(data.validity_msg_L2) }}</h4>
                            <h4
                            >{{$t('sim_card_number')}} {{data.sim_card_number ?? '-'}}</h4>
                            <h4>{{ $t('Software version') }}: {{ data.soft_version?.split('').join('.') }}</h4>
                            <h4>{{ $t('Hardware version') }}: {{ data.hardware_version?.split('').join('.') }}</h4>
                            <h4 >{{$t('Last update')}}:{{data.last_update && data.last_update.updated_at ? formatDateToCustomFormat(new Date(data.last_update.updated_at)) : '-' }}</h4>
                        </template>
                            ...
                            <div v-if="data.errors.length !== 0">
                                <ul>
                                    <li v-for="error in data.errors">
                                        <b>
                                            {{ formatDateToCustomFormat(new Date(error.created_at)) }} -
                                        </b>
                                        {{ error.errorText }}
                                        <v-icon color="red" @click="deleteError(error.id)"> mdi-delete</v-icon>
                                    </li>
                                </ul>
                            </div>
                            <div v-else>
                ...
                <UserTable :deviceId="data.id" @loadDevice="loadItems" :is-fixed="data.op_mode == 0"
                           :server-items="data.users"></UserTable>
                      ...
                      >
                        ...
                        >
                            <v-text-field
                                ...
                                class="text-capitalize"
                                :label="$t('The first line of the guest message')"
                        ...
                        >
                            <v-text-field
                                ...
                                class="text-capitalize"
                                :label="$t('The second line of the guest message')"
                                :rules="[maxLengthRule15]"
                        ...
                        >
                            <v-text-field
                                ...
                                class="text-capitalize"
                                :label="$t('The first line of the validation message')"
                                :rules="[maxLengthRule15]"
                                required
                            ></v-text-field>
                        </v-col>
                        ...
                        >
                            <v-text-field
                                ...
                                class="text-capitalize"
                                :label="$t('The second line of the validation message')"
                                :rules="[maxLengthRule15]"
                                required
                            ></v-text-field>
                        </v-col>
                        <v-col
                            ...
                            cols="11"
                        >
                            <span>{{ $t('LCD brightness') }}: {{ editedItem.lcd_brightness }}</span>
                            <v-slider
                        ...
                        <v-col
                            ...
                            cols="11"
                        >
                            <span>{{ $t('LED brightness') }}: {{ editedItem.led_brightness }}</span>
                            <v-slider
                        ...
                        <v-col
                            ...
                            cols="11"
                        >
                            <span>{{ $t('Message display time') }}: {{ editedItem.msg_appear_time }}</span>
                            <v-slider
                        ...
                        <v-col
                            ...
                            cols="11"
                        >
                            <span>{{ $t('Relay pulse time') }}: {{ editedItem.relay_pulse_time }}</span>
                            <v-slider
                        ...
                        <v-col
                            ...
                            cols="11"
                        >
                            <span>{{ $t('Card reading time') }}: {{ editedItem.card_read_delay }}</span>
                            <v-slider
                        ...
                        <v-col
                            ...
                            cols="11"
                        >
                            <span>{{ $t('Number of cards per user') }}: {{ editedItem.limit }}</span>
                            <v-slider
...
export default {
    components: {VDataTable, SignalIcon, UserTable, EarningTable, WithdrawalsTable},
    name: 'detailPage',
    data: () => ({
        search: '',
        dialog: false,
            ...
            "errors": [
                {
                    ...
                    "device_id": 3,
                    "errorCode": "746",
                    "errorText": "This is a mock error text for device 1",
                    "created_at": "2023-09-18T02:10:52.000000Z",
                    "updated_at": "2023-09-18T02:10:52.000000Z"
                },
                {
                    "id": 7,
                    "device_id": 3,
    ...
    methods: {
        ...
        loadItems(withCong = false) {
            axios.get('/api/devices/' + this.$route.params.id).then(({data}) => {
                this.$nextTick(() => {
        ...
        resetDevice(item) {
            this.$swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.put('/api/devices/' + item.id + '/reset', {
                        ...item, ...this.resetItem,
        ...
        deleteError(id) {
            this.$swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    axios.delete('/api/device/error/' + id).then(() => this.loadItems());
                    this.$swal.fire({
                        icon: 'success',
                        position:'center',
                        allowOutsideClick: false,
