                                ...
                                if (empty($user)) {
                                    ...
                                    if ($device->op_mode == 0) {
                                        ...
                                        if (time() < Carbon::parse($userDevice->subscription)->timestamp) {
                                            ...
                                            // 
                                            $fixedCardAmount = Card::where("user_id", $user->id);
                                            $cardAmount = $user->fixed_card_amount * $fixedCardAmount ;
                                            $combinedTarrifAmount =  $device->tariff_amount +   $cardAmount;
                                            if ($user->balance >  $combinedTarrifAmount ) {
                                                
                                                $payload = $this->generateHexPayload(1, []);
                                                $this->publishMessage($device_id, $payload,);
                                                $user->balance = $user->balance -  $combinedTarrifAmount ;
                                                $user->save();
                                                $this->saveOrUpdateEarnings($device->id, $combinedTarrifAmount , $device->company_id);
                                            } else {
                                                $this->noMoney($device_id);
                                            }
                                        }
                                    ...
                                    if (empty($card)) {
                                        ...
                                        if ($device->op_mode == 0) {
                                            ...
                                            if (time() < Carbon::parse($userDevice->subscription)->timestamp) {
                                                ...
                                                $cardAmount = $user->fixed_card_amount * $fixedCardAmount ;
                                                $combinedTarrifAmount =  $device->tariff_amount +   $cardAmount;
                                                if (($user->balance - $user->freezed_balance) >=  $combinedTarrifAmount ) {
