<template>
  <div>
    <v-card class="mb-3">
      ...
      <v-card-text class="mb-0">
        ...
        <v-card-actions class="justify-end">
          <v-btn color="primary" v-on:click="submitFile()">
            {{ $t('save') }}
          </v-btn>
        </v-card-actions>
      </v-card-text>
    ...
    <v-card class="mx-auto mb-3">
      ...
      <v-divider></v-divider>
      <v-virtual-scroll :items="files" height="200" item-height="48">
        <template v-slot:default="{ item }">
          <v-list-item :title="item.filename" :subtitle="item.version">
        ...
        <v-row class="justify-space-between">
          ...
          <v-col v-if="seriesB[0] + seriesB[1] + seriesB[2]" cols="12" md="6">
            <v-card style="height: 100%;" class="overflow-auto pa-2">
              <h3>
                {{ $t('განახლებისთვის გამზადებული ლიფტების მდგომარეობა') }}
              </h3>
              <h4>
                {{ $t('Total number of elevators') }}:{{
                  seriesB[0] + seriesB[1] + seriesB[2]
                }}
              </h4>
              <apexchart
                width="400"
        ...
        <template v-slot:top>
          <v-toolbar>
            ...
            <v-menu v-if="$vuetify.display.smAndDown">
              ...
              <v-card width="200" class="pa-0 ma-0">
                <v-btn
                  ...
                  @click="sendUpdateSelectedDevices"
                >
                  {{ $t('Update software') }}
                </v-btn>
                <v-btn
            ...
            <template v-if="$vuetify.display.mdAndUp">
              ...
              <v-btn color="red" v-if="seriesB[2]" @click="checkFailed">
                {{ $t('Clear failed') }}
              </v-btn>
              <v-btn color="red" v-if="seriesB[1]" @click="checkSuccess">
                {{ $t('Clear success') }}
              </v-btn>
        ...
        <template v-slot:item="{ item }">
          <tr>
            ...
            <td>
              ...
              <router-link :to="'/devices/' + item.raw.device_id">
                {{ item.raw.dev_id }}
              </router-link>
            </td>
            <td>
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        {
          title: this.$t('Old version'),
          key: 'old_version',
        },
        {
          title: this.$t('New version'),
          key: 'new_version',
        },
        {
      ...
      this.$swal
        ...
        .then((result) => {
          if (result.isConfirmed) {
            axios
              .post('/api/send/update/to/selected/devices', {
    ...
    checkSuccess() {
      this.$swal
        ...
        .then((result) => {
          if (result.isConfirmed) {
            axios
              .get('/api/updating-device/check-success/last-created')
              .then(() => {
                this.$swal.fire({
                  icon: 'success',
                  position: 'center',
    ...
    checkFailed() {
      this.$swal
        ...
        .then((result) => {
          ...
          if (result.isConfirmed) {
            axios
              .get('/api/updating-device/check-failed/last-created')
              .then(() => {
                this.$swal.fire({
                  icon: 'success',
                  position: 'center',
